...
for s in solv:
    for m in mult:
        logreg = LogisticRegression(solver=s,multi_class=m)# calling the classifier 
        logreg.fit(df1[cols],df1.survived) #fitting the data to the model
        ypred=logreg.predict(test[cols]) # prediction based on the model 
        score=0;
        for x in range(263):
            if(ypred[x]==test.survived[x]): # if prediction equals to test data we have a win. 
                score=score+1
                                                    
        print('Parameters: Solver:',s,' Multiclass',m)
        print('Accuracy:',(score/263)*100,'% accuracy')
